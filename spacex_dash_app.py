...
def update_pie_chart(selected_site):
    if selected_site == 'ALL': 
        fig = px.pie(spacex_df, names='Launch Site', values='class', title='Success Count for All Launch Sites')
    else:
...
def update_scatter_chart(selected_site, payload_range):
    if selected_site == 'ALL':
        ...
        title = f'Scatter plot for All Launch Sites ({payload_range[0]} - {payload_range[1]} Kg)'
    else:
        ...
        title = f'Scatter plot for {selected_site} ({payload_range[0]} - {payload_range[1]} Kg)'
